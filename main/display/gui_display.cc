 ...
 * @Date               : 2025-03-23 09:22:29
 * @LastEditors        : Felix
 * @LastEditTime       : 2025-03-23 09:28:58
 */
...
SpiGuiDisplay::SpiGuiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                             int width, int height, int offset_x, int offset_y, bool mirror_x, bool mirror_y, bool swap_xy,
                             DisplayFonts fonts)
    : GuiDisplay(panel_io, panel, fonts)
{
    width_ = width;
    height_ = height;
...
// RGB LCD实现
RgbGuiDisplay::RgbGuiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
...
bool GuiDisplay::Lock(int timeout_ms)
{
    return lvgl_port_lock(timeout_ms);
}

void GuiDisplay::Unlock()
{
    lvgl_port_unlock();
}

#if CONFIG_USE_WECHAT_MESSAGE_STYLE
void GuiDisplay::SetupUI()
{
...
void GuiDisplay::SetChatMessage(const char *role, const char *content)
{
    DisplayLockGuard lock(this);
    if (content_ == nullptr)
    {
...
void GuiDisplay::SetClockUI(lv_obj_t *parent)
{
    // 创建时钟界面
...
void GuiDisplay::SetIcon(const char *icon)
{
    DisplayLockGuard lock(this);
    if (emotion_label_ == nullptr)
    {
...
void GuiDisplay::SetTheme(const std::string &theme_name)
{
    DisplayLockGuard lock(this);
